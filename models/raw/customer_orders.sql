{{ config(
    ...
    post_hook=[
        """
        DELETE FROM SUPPLY_CHAIN_DB.LANDING.RAW_CUSTOMER_ORDER
        WHERE LOAD_TS < DATEADD(DAY, -90, CURRENT_DATE);
        """,
        """
               INSERT INTO SUPPLY_CHAIN_DB.AUDIT.MODEL_EXECUTION_LOG (
                    model_name,
                    load_date,
                    row_count,
                    file_name,
                ...
                FROM SUPPLY_CHAIN_DB.LANDING.RAW_CUSTOMER_ORDER
                WHERE CAST(LOAD_TS AS DATE) = CURRENT_DATE
                AND STG_FILE_NAME IS NOT NULL
                AND STG_FILE_NAME NOT IN (
                    SELECT file_name
                    FROM SUPPLY_CHAIN_DB.AUDIT.MODEL_EXECUTION_LOG
